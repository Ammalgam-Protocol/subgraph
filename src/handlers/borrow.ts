...
indexer.onEvent({ contract: 'ERC4626Debt', event: 'Borrow' }, async ({ event, context }) => {
  ...
  const updatedPool = {
    ...
    borrowCount: pool.borrowCount + 1,
  ...
  // Calculate principal adjustment (subtract for borrow)
  ...
  context.User.set({ ...user, borrowCount: user.borrowCount + 1 })
...
indexer.onEvent({ contract: 'ERC4626Debt', event: 'Repay' }, async ({ event, context }) => {
  ...
  // Peripheral check on `onBehalfOf`
  let userAddress: string
  if (config.peripheralAddresses.includes(event.params.onBehalfOf.toLowerCase())) {
    userAddress = event.transaction.from!
  } else {
    userAddress = event.params.onBehalfOf
  }
  const userId = scopedId(event.chainId, userAddress)
  ...
  // Calculate principal adjustment (add for repay)
  const activeLiquidity = updatedPool.totalAssets[DEPOSIT_L] - updatedPool.totalAssets[BORROW_L]
  let principal = ZERO_BI
  if (tokenType === DEPOSIT_X) {
    principal = convertXToL(event.params.assets, pool.reserveX, activeLiquidity)
  ...
  context.Pool.set(updatedPool)
  context.Position.set(updatedPosition)
  context.User.set({ ...user, repayCount: user.repayCount + 1 })
...
indexer.onEvent({ contract: 'ERC4626Debt', event: 'Transfer' }, async ({ event, context }) => {
  const config = getChainConfig(event.chainId)
