...
export function handleBorrowLiquidity(event: BorrowLiquidityEvent): void {
  ...
  if (pool) {
    ...
    pool.totalAssets = update<BigInt>(
      pool.totalAssets,
      pool.totalAssets[tokenType].plus(event.params.borrowAmountLShares),
      tokenType,
    )
    pool.totalShares = update<BigInt>(
      pool.totalShares,
      pool.totalShares[tokenType].plus(event.params.borrowAmountLShares),
      tokenType,
    )
    ...
    position.assets = update<BigInt>(
      position.assets,
      position.assets[tokenType].plus(event.params.borrowAmountLShares),
      tokenType,
    )
    position.shares = update<BigInt>(
      position.shares,
      position.shares[tokenType].plus(event.params.borrowAmountLShares),
      tokenType,
    )

    // Update position principal balance
    // TODO: Update it to `assets` after deploying latest core contracts
    position.principal = position.principal.minus(event.params.borrowAmountLShares)
    ...
    // TODO: Update it to `assets` after deploying latest core contracts
    borrow.amount = event.params.borrowAmountLShares
    borrow.shares = event.params.borrowAmountLShares
    borrow.pool = pool.id
    borrow.user = user.id
